...
io.on('connection', function(sock) {
	...
	sock.on('message', function(msgStr){
  	...
  	sock.on('new subject', function (sbj) {
		dm.addSubject(sbj, function(x){
			console.log("Event: new subject: " + sbj + '-->' + x);
			if (x == -1) {
				sock.emit ('new subject', 'err', 'El tema ya existe', sbj);
			} else {
				sock.emit ('new subject', 'ack', x, sbj);
			   io.emit ('new subject', 'add', x, sbj); 			
			}
		});
  	});
  	...
  	sock.on ('get message list', function (from, to, isPriv) {
		...
		if (isPriv) {
			// dm.getPrivateMessageList(function(x){
			// 	sock.emit ('message list', from, to, isPriv, x);
			// })
	  	} else {
			dm.getPublicMessageList(to, function(x){
				sock.emit ('message list', from, to, isPriv, x);
			});
	  	}
  	});
